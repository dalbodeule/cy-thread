...
The app's shared visual system lives in `app/assets/css/main.css`; the homepage is under `app/pages/index.vue`. Dedicated pages are available at `/login`, `/forums/<slug>/threads`, `/forums/<slug>/threads/<id>`, `/forums/<slug>/admin`, `/forums/<slug>/admin/categories`, and `/forums/<slug>/reports`. The thread detail page supports replies, reports, and moderator pin/lock/hide actions. The admin page manages moderators and links to category settings; the report page supports review, dismissal, resolution, and author bans. Only community owners can appoint admins; owners and admins can appoint or remove moderators, enforced by server routes as well as the interface.
